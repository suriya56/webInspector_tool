...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('ai-ui-editor-bridge-windsurf.connect', () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('ai-ui-editor-bridge-windsurf.disconnect', () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('ai-ui-editor-bridge-windsurf.openCascade', () => {
      openCascadeChat();
    })
  );
...
function updateStatusBar(connected: boolean) {
  ...
  if (connected) {
    ...
    statusBarItem.tooltip = 'Not connected to FastAPI Bridge. Click to connect.';
    statusBarItem.command = 'ai-ui-editor-bridge-windsurf.connect';
...
function connectToBridge(context: vscode.ExtensionContext) {
  ...
  const serverUrl = config.get<string>('serverUrl') || 'ws://localhost:8000/ws/windsurf';
...
async function handleBrowserContext(payload: BrowserPayload) {
  ...
  try {
    ...
    vscode.window.showInformationMessage(
      `AI UI Bridge (Windsurf): Context ready for ${filePath}`,
      'Copy Context'
    ).then(selection => {
...
function buildContextMessage(payload: BrowserPayload): string {
  ...
  message += '```html\n';
  message += outerHTML.substring(0, 2000) + (outerHTML.length > 2000 ? '...' : '');
  message += '\n```\n\n';

  message += `### Computed Styles\n`;
  message += '```json\n';
  message += JSON.stringify(computedCSS, null, 2).substring(0, 1000) + (JSON.stringify(computedCSS).length > 1000 ? '...' : '');
  message += '\n```\n\n';
...
async function openFileAndShowCascade(filePath: string, contextMessage: string) {
    ...
    try {
      ...
      try {
        ...
        console.log('Cascade context:\n\n' + buildContextMessage({
          prompt: '', outerHTML: '', computedCSS: {}, filePath: '', tagName: '', className: '', id: ''
        }));
        vscode.window.showWarningMessage(
          'Could not open Cascade automatically. Check the Output panel for context.'
        );
      }
    }
  }
