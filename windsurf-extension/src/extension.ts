...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "ai-ui-editor-bridge-windsurf.connect",
      () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "ai-ui-editor-bridge-windsurf.disconnect",
      () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "ai-ui-editor-bridge-windsurf.openCascade",
      () => {
        openCascadeChat();
      },
    ),
...
function connectToBridge(context: vscode.ExtensionContext) {
  ...
  const serverUrl =
    config.get<string>("serverUrl") || "ws://localhost:8000/ws/windsurf";

  if (webSocket && webSocket.readyState === WebSocket.OPEN) {
    vscode.window.showInformationMessage(
      "AI UI Bridge (Windsurf): Already connected",
    );
    return;
  }
  ...
  try {
    ...
    webSocket.onopen = () => {
      ...
      vscode.window.showInformationMessage(
        "AI UI Bridge (Windsurf): Connected to Cascade",
      );
    ...
    webSocket.onmessage = (event) => {
      try {
        ...
        vscode.window.showErrorMessage(
          "AI UI Bridge (Windsurf): Invalid message format",
        );
      }
    };
    ...
    webSocket.onerror = (error) => {
      ...
      vscode.window.showErrorMessage(
        "AI UI Bridge (Windsurf): Connection error",
      );
      updateStatusBar(false);
    };
    ...
    webSocket.onclose = (event) => {
      console.log(
        "Disconnected from FastAPI Bridge:",
        event.code,
        event.reason,
      );
    ...
    vscode.window.showErrorMessage(
      `AI UI Bridge (Windsurf): Failed to connect - ${error}`,
    );
    updateStatusBar(false);
  }
}
...
function disconnectFromBridge() {
  if (webSocket) {
    ...
    webSocket = null;
    vscode.window.showInformationMessage(
      "AI UI Bridge (Windsurf): Disconnected",
    );
    updateStatusBar(false);
  }
}
    ...
    vscode.window
      ...
      .then((selection) => {
        if (selection === "Copy Context") {
          ...
          vscode.window.showInformationMessage(
            "Context copied! Paste into Cascade chat (Ctrl+V)",
          );
        }
      });
  } catch (error) {
...
async function openFileAndShowCascade(
  filePath: string,
